  ...
  const slides = [
    {
      ...
      illustration: (
        <div className={`w-full h-60 rounded-2xl flex items-center justify-center relative overflow-hidden ${
          ...
          {isWireframe ? (
            ...
            <>
              ...
              <div className="absolute bottom-6 left-6 animate-pulse">
                <AlertTriangle className="w-6 h-6 text-amber-200/80" />
              </div>

              {/* Stats overlay */}
              <div className="relative z-10 flex gap-4">
                <div className="bg-black/30 backdrop-blur rounded-xl p-4 text-center text-white min-w-[100px]">
                  <span className="text-3xl font-black font-display block">300</span>
                  <span className="text-[9px] uppercase tracking-wider opacity-80">Ton / Hari</span>
                </div>
                <div className="bg-black/30 backdrop-blur rounded-xl p-4 text-center text-white min-w-[100px]">
                  <span className="text-3xl font-black font-display block">15m</span>
                  <span className="text-[9px] uppercase tracking-wider opacity-80">Gunungan Sampah</span>
                </div>
              </div>

              <p className="absolute bottom-3 text-[10px] text-white/70 font-semibold tracking-wider">
          ...
          <div className={`w-full h-60 rounded-2xl flex items-center justify-center relative overflow-hidden ${
            ...
            {isWireframe ? (
              ...
              <>
                ...
                {/* Sorting bins visual */}
                <div className="relative z-10 grid grid-cols-4 gap-3 px-4">
                  {[
                    ...
                    return (
                      <div key={i} className="flex flex-col items-center gap-1.5">
                        <div className={`${bin.color} w-14 h-14 rounded-xl flex items-center justify-center shadow-lg shadow-black/20`}>
                          <BinIcon className="w-7 h-7 text-white" />
                        </div>
                        <span className="text-[8px] text-white font-bold uppercase tracking-wider">{bin.label}</span>
                      </div>
                    );
                  })}
                </div>

                <div className="absolute bottom-3 flex items-center gap-1.5 bg-white/20 backdrop-blur rounded-full px-4 py-1.5">
                  <TrendingUp className="w-3.5 h-3.5 text-emerald-200" />
                  <span className="text-[9px] text-white font-bold">Tukarkan Jadi Voucher & BST</span>
                </div>
              </>
            )}
          </div>
              ...
              <>
                ...
                <div className="relative z-10 flex flex-col items-center gap-4">
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div className="text-white text-3xl font-black animate-pulse">⟶</div>
                    <div className="bg-emerald-400/30 backdrop-blur rounded-2xl p-5 border border-emerald-300/30">
                  ...
                  <div className="flex gap-2.5">
                    {["Foto", "GPS", "Kirim", "Bersih"].map((s, i) => (
                      <div key={i} className="flex items-center gap-1 bg-white/15 backdrop-blur rounded-full px-3 py-1.5">
                ...
                <p className="absolute bottom-3 text-[9px] text-blue-200/70 font-semibold tracking-wider">
      ...
      <div className={`flex-1 flex flex-col p-6 overflow-y-auto ${isWireframe ? 'bg-white text-gray-800' : 'bg-white'}`}>
        ...
        <div className="flex justify-between items-center shrink-0">
          <div className="flex items-center gap-2">
            <img src={logo} alt="Solo Pilah" className="w-6 h-6 object-contain" />
            <span className={`text-base font-extrabold font-display ${isWireframe ? 'text-gray-800' : 'text-emerald-600'}`}>Solo Pilah</span>
          </div>
          <button
            onClick={onComplete}
        ...
        <div className="flex-1 flex flex-col justify-center space-y-6 py-6 min-h-0">
          ...
          <div className="space-y-3">
            <h2 className={`text-2xl font-bold font-display leading-snug ${isWireframe ? 'text-gray-900 border-b border-gray-200 pb-1' : 'text-gray-800'}`}>
              {currentSlide.title}
            </h2>
            <p className={`text-sm leading-relaxed ${isWireframe ? 'text-gray-500' : 'text-gray-600'}`}>
        ...
        <div className="space-y-5 shrink-0">
          ...
          <button
            ...
            className={`w-full py-4 rounded-xl font-bold text-base flex items-center justify-center gap-2 shadow-sm cursor-pointer transition-all ${
              isWireframe
